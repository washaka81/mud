...
# Detección dinámica de dispositivo (CUDA > CPU)
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
if DEVICE == "cuda":
    # T4 tiene compute capability 7.5. BF16 requiere 8.0+.
    device_cap = torch.cuda.get_device_capability(0)
    SUPPORTS_BF16 = device_cap[0] >= 8
else:
    SUPPORTS_BF16 = False

# Forzar FP16 si se solicita o si la GPU no soporta BF16 nativo
USE_FP16 = os.environ.get("MUD_USE_FP16") == "1" or (DEVICE == "cuda" and not SUPPORTS_BF16)
...
class MoELayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Componentes de pérdida calculados de forma eficiente para evitar graph breaks
        importance   = probs.view(-1, self.n_experts).mean(0)
        loss_imp     = importance.var() * self.aux_coeff * self.n_experts

        flat_i = topk_i.view(-1, self.top_k)
        # Load balance loss (estimación vectorial)
        # Usamos bincount si es posible, o una aproximación sumando one_hot
        load = F.one_hot(flat_i, num_classes=self.n_experts).float().mean(dim=(0, 1))
        ...
        # Actualizar telemetría de clústeres (en no_grad para evitar backprop)
        if not self.training or random.random() < 0.1: # Reducir frecuencia de update en training
            with torch.no_grad():
                cluster_ids = topk_i // self.cluster_size
                # Usar view(-1) para aplanar y facilitar sumas
                self.cluster_activations.index_add_(
                    0, 
                    cluster_ids.view(-1), 
                    torch.ones(cluster_ids.numel(), dtype=torch.long, device=x.device)
                )

        # Dispatch vectorizado
        flat_x = xn.view(-1, C)
        flat_i = topk_i.view(-1, self.top_k)
        flat_p = topk_p.view(-1, self.top_k)

        out = torch.zeros_like(flat_x)
        
        # Identificar qué expertos están activos en este lote para evitar el loop completo
        active_experts = torch.unique(flat_i)
        
        for e_idx in active_experts.tolist():
            expert = self.experts[e_idx]
            # Encontrar tokens que usan este experto
            ...
            # mask[token_mask] es [N_active, K], flat_p[token_mask] es [N_active, K]
...
def train():
    ...
    parser.add_argument("--batch-size", type=int, default=_cfg.get("batch_size", 4),
                        help="Tamaño del lote (batch size) para entrenamiento")
    ...
    print(f"🔧 Parámetros totales: {n_params:.1f}M | Activos/token: {n_active:.2f}M | Device: {DEVICE}")
    ...
    try:
        ...
        optimizer = torch.optim.AdamW(
            ...
            betas=(0.9, 0.95), weight_decay=0.1, fused=use_fused,
        )
    ...
    # ── Checkpoint resume ──────────────────────────────────────────────────────
    # Soporte Colab / Kaggle: rutas absolutas via variable de entorno
    _models_dir = os.environ.get("MUD_MODELS_DIR", "models")
    os.makedirs(_models_dir, exist_ok=True)
    ckpt_path   = os.environ.get("MUD_CKPT_PATH",
                                 os.path.join(_models_dir, "mud_fast_ckpt.pt"))
    start_step = 0
    ...
    if _total_gb > AVAILABLE_RAM_GB * 0.7:
        ...
        print(f"   ⚠️  Usa --experts N y --steps pequeños si hay OOM")
    print()
    ...
    for step in pbar:
        ...
        # Padding simple para el lote
        x_batch = torch.nn.utils.rnn.pad_sequence(batch_ids, batch_first=True, padding_value=0)
        x_ids  = x_batch[:, :-1].to(DEVICE)
        target = x_batch[:, 1:].to(DEVICE)
        ...
        for module in model.modules():
            if isinstance(module, MoELayer):
                module._step_ratio = torch.tensor(step_ratio, device=DEVICE)

        # Neural Kick (Epsilon Jitter) cada 100 pasos para evitar colapso de expertos
        ...
        # Forward — Autocast (FP16 para T4, BF16 para Ampere/CPU si habilitado)
        autocast_device = "cuda" if DEVICE == "cuda" else "cpu"
        autocast_dtype = torch.float16 if USE_FP16 else torch.bfloat16
        autocast_enabled = args.bf16 or USE_FP16

        with torch.autocast(autocast_device, dtype=autocast_dtype, enabled=autocast_enabled):
            logits, balance_loss = model(x_ids)
            # Aplicar temperatura al gate softmax si es necesario (aquí ya está dentro de MoELayer)
            # logits: [B, T, V]
        ...
        # Checkpoint de hito cada 5000 pasos (numerado, para recuperación de sesión)
        if (step + 1) % 5000 == 0:
            raw_model = model._orig_mod if hasattr(model, "_orig_mod") else model
            milestone_path = os.path.join(
                _models_dir, f"mud_ckpt_step{step+1:07d}.pt"
            )
            torch.save({
                "step":        step + 1,
                "model":       raw_model.state_dict(),
            ...
            print(f"   🏁 Checkpoint de hito → {milestone_path}")
            # Mantener solo los últimos 3 hitos para ahorrar espacio
            import glob
            hitos = sorted(glob.glob(os.path.join(_models_dir, "mud_ckpt_step*.pt")))
            for old in hitos[:-3]:
                os.remove(old)
                print(f"   🗑️  Hito antiguo eliminado: {os.path.basename(old)}")
    ...
    mud_out   = os.environ.get("MUD_EXPORT_PATH",
                               os.path.join(_models_dir, "core_skills.mud"))
    raw_model = model._orig_mod if hasattr(model, "_orig_mod") else model
