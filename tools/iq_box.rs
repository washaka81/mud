...
fn main() {
    ...
    let label = args
        .get(2)
        .cloned()
        .unwrap_or_else(|| "COGNICIÓN FRAGMENTADA".to_string());
    ...
    let iq_color = if iq_val < 15.0 {
        Color::Yellow
    } else if iq_val < 100.0 {
        Color::Cyan
    } else {
        Color::Green
    };
    ...
    table.add_row(vec![Cell::new("STATE"), Cell::new(label).fg(iq_color)]);
