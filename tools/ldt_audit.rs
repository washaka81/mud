use forge_llm::mud::inference::InferenceWorkspace;
...
fn main() -> anyhow::Result<()> {
    ...
    let hidden = 3072;
    let ws = InferenceWorkspace::new(None, hidden, 4096, 32, 8, 200000, 16, 4);
    ...
    println!("   [PRE-PROJECTION] Latent state sampled at [0, 1, 2, 3]:");
    {
        ...
        println!("     - H0: {:.4}, H1: {:.4}, H2: {:.4}, H3: {:.4}", x_guard[0], x_guard[1], x_guard[2], x_guard[3]);
    }
    ...
    assert!(!converged_drift, "Should NOT trigger early exit on shifted states");

    println!("✅ LDT-02 Validation Passed: L2 Euclidean Shift correctly terminates reasoning loops.\n");
