...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "{}🛡️  MUD ENGINE HIGH-FIDELITY INTERACTIVE VALIDATOR 🛡️{}",
        BOLD, RESET
    );
    println!(
        "{}=================================================={}",
    ...
    println!(
        "\n{}--- STAGE 1: TOKENIZER & BPE CONCORDANCE AUDIT ---{}",
        BOLD, RESET
    );
    ...
    println!(
        "   Vocab size in metadata: {} tokens",
        tokens_str.lines().count()
    );
    println!(
        "   BPE merges in metadata: {} rules",
    ...
    if tokenizer_ok {
        println!(
            "{}   ✅ TOKENIZER VERIFICATION: PASSED (Concordance is healthy){}",
            GREEN, RESET
        );
    } else {
    ...
    println!(
        "\n{}--- STAGE 2: MOE EXPERTS WEIGHT & SIGMA ANATOMY ---{}",
        BOLD, RESET
    );
        ...
        if tensor.t_type == MudTensorType::Ternary2Bit {
            ...
            if name.starts_with("blk.") {
                ...
                if let Ok(layer_idx) = parts[1].parse::<usize>() {
                    layer_sigmas.entry(layer_idx).or_default().push(sigma);
                }
            }
    ...
    println!(
        "\n   {}Mean Sigma per Layer (Active MoE Routing Health):{}",
        CYAN, RESET
    );
    ...
    if nan_count == 0 && dead_layers.is_empty() {
        println!(
            "\n{}{}   ✅ WEIGHT DIAGNOSTICS: PERFECT (All weights healthy and stabilized){}",
            GREEN, BOLD, RESET
        );
    } else {
        if nan_count > 0 {
            println!(
                "\n{}{}   ❌ WEIGHT DIAGNOSTICS: FAILED (Found {} NaN/Inf weight values!){}",
                RED, BOLD, nan_count, RESET
            );
        }
        if !dead_layers.is_empty() {
            println!(
                "{}{}   ❌ WEIGHT DIAGNOSTICS: FAILED (Collapsed weights in layers: {:?}){}",
                RED, BOLD, dead_layers, RESET
            );
        }
    }

    println!(
        "\n{}--- STAGE 3: KV ATTENTION & CONTEXT STRESS TEST ---{}",
        BOLD, RESET
    );
    ...
    for step in 0..5 {
        ...
        println!(
            "      Step {:>2} | L2 Norm: {:.4} | Finite: {}",
            step, l2_norm, !has_nan
        );
    }

    if stable {
        println!(
            "{}   ✅ KV ATTENTION TEST: PASSED (Context attention propagation is highly stable){}",
            GREEN, RESET
        );
    } else {
        println!(
            "{}   ❌ KV ATTENTION TEST: FAILED (Attention output collapsed or contains NaNs){}",
            RED, RESET
        );
    }
    ...
    println!(
        "{}🎉 MUD MODEL INTEGRITY VERIFICATION COMPLETE 🎉{}",
        BOLD, RESET
    );
    println!(
        "{}=================================================={}",
