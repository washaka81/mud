...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "   - Matriz de Pesos: {:.2} MB",
        (weights.len() * 4) as f64 / 1_048_576.0
    );
    ...
    let compute_duration =
        std::time::Duration::from_secs_f64(compute_time_secs) + dispatch_overhead;

    let gpu_total = compute_duration * vulkan_iters;

    println!(
        "   - Tiempo total ({} iteraciones): {:?}",
        vulkan_iters, gpu_total
    );
    println!("   - Latencia promedio por experto: {:?}", compute_duration);
    println!(
        "   - Overhead de Despacho (Zero-Copy): {:?}",
        dispatch_overhead
    );
    ...
    println!(
        "   Aceleración Teórica de Vulkan: {:.2}x frente a P-Cores AVX2",
        speedup
    );
