use forge_llm::mud::inference::{MudInference, MudLayer};
...
fn main() -> anyhow::Result<()> {
    ...
    if n_experts != NUM_EXPERTS {
        println!(
            "\x1b[1;33m  ⚠ NOTA: El modelo tiene {} expertos (esperados {}). \
    ...
    for c in 0..NUM_CLUSTERS {
        ...
        let (m1, m2, m3) = if counted > 0 {
            (
                w1_sum / counted as f32,
                w2_sum / counted as f32,
                w3_sum / counted as f32,
            )
        } else {
            (0.0, 0.0, 0.0)
        };
        ...
        let n_loaded = if counted > 0 {
            expert_end - expert_start
        } else {
            0
        };
        println!(
            "{}{:>2}  {:<28} {:>6.4} {:>6.4} {:>6.4}  {}{}  ({}/{})\x1b[0m",
            color,
            c + 1,
            CLUSTER_NAMES[c],
            m1,
    ...
    let mut cluster_activations = [0usize; NUM_CLUSTERS];
    ...
    for prompt in &test_prompts {
        ...
        if let Some(MudLayer::Attention(layer)) = engine.model.layers.first() {
            // Calcular gate logits manualmente usando el campo expuesto
            let ws_gate = &engine.workspace.gate_logits;
            ws_gate.fill(0.0);

            let x_unified = forge_llm::mud::inference::UnifiedBuffer::new_cpu_from_slice(&x);
            ...
            let mut routing_indexed = Vec::with_capacity(engine.model.num_experts);
            let mut routing_results = Vec::with_capacity(8);
            layer.router.route_in_place(&ws_gate.read(), &mut routing_indexed, &mut routing_results);
            for (expert_id, _prob) in &routing_results {
                let cluster_id: usize = (expert_id * NUM_CLUSTERS) / n_experts;
    ...
    for c in 0..NUM_CLUSTERS {
        ...
        let pct = if total_activations > 0 {
            count as f32 * 100.0 / total_activations as f32
        } else {
            0.0
        };
    ...
    println!(
        "\n  📊 Score de Balance: {:.1}% (desviación máxima: {:.1}%)",
        balance_score, max_dev
    );
    ...
    if nan_count == 0 {
        ...
        println!(
            "  \x1b[1;31m❌ Inestabilidad: {} pasos con NaN/Inf detectados\x1b[0m",
            nan_count
        );
    }
    ...
    println!(
        "  Expertos:    {} → {} clústeres × {} micro-expertos",
        n_experts, NUM_CLUSTERS, CLUSTER_SIZE
    );
    println!(
        "  Saludables:  {} clústeres  |  Débiles: {}  |  Muertos: {}",
...
fn magnitude(skill: &forge_llm::mud::MudSkill, name: &str) -> f32 {
    let Some(t) = skill.tensors.get(name) else {
        return 0.0;
    };
