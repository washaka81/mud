        ...
        if name.contains(query) {
            ...
            match tensor.t_type {
                MudTensorType::Float32 => {
                    let slice = unsafe {
                        std::slice::from_raw_parts(tensor.data_ptr as *const f32, elements)
                    };
                    ...
                    println!(
                        "   Sparsity: {:.2}%",
                        (zeros as f32 / elements as f32) * 100.0
                    );
                ...
                MudTensorType::Float16 => {
                    println!(
                        "   Error: Microscope currently does not support f16 decoding natively."
                    );
                }
                MudTensorType::Int4 => {
                    println!(
