...
fn main() -> anyhow::Result<()> {
    ...
    if args.len() < 3 {
        ...
        println!("Example: mud_calibrator models/smollm2_mud.mud blk.0.expert.0.w1.prq_scale 0.05");
        return Ok(());
    }
    ...
    if tensor.t_type != MudTensorType::Float32 {
        println!(
            "Error: Calibration tool currently targets FP32 scaling/norm tensors. This is {:?}",
            tensor.t_type
        );
        return Ok(());
    }
    ...
    if args.len() == 4 {
        ...
        println!(
            "Applying nanometric adjustment: {} -> {}",
            slice[0], new_val
        );
        ...
        if file_mut
            .seek(SeekFrom::Start(abs_file_offset as u64))
            .is_ok()
        {
            let new_data = vec![new_val; elements];
            let bytes = unsafe {
                std::slice::from_raw_parts(new_data.as_ptr() as *const u8, new_data.len() * 4)
            };
        ...
        println!(
            "Tip: If logit variance is too high, lower the w2.prq_scale or attn_output.prq_scale."
        );
    }
