...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(core) = mud.skills.get("core") {
        for (name, tensor) in &core.tensors {
            if tensor.t_type == MudTensorType::Ternary2Bit {
                ...
                zero_weights +=
                    (local_zero as f32 * (u32_count as f32 / sample_size as f32)) as usize;
            } else if name.ends_with(".prq_scale") {
    ...
    if ternary_weights == 0 {
        ternary_weights = 1;
    }
    let avg_sparsity = (zero_weights as f32 / ternary_weights as f32) * 100.0;
    let model_certainty = (100.0 - ((avg_sparsity - 26.0).abs() / 100.0 * 100.0)).clamp(0.0, 100.0);
    let avg_scale = if scale_count > 0 {
        scales_sum / scale_count as f32
    } else {
        0.0
    };
    ...
    table
        ...
        .set_header(vec![
            Cell::new("MUD DETERMINISTIC RECALIBRATION PROJECTOR")
    ...
    table.add_row(vec![
        "Ternary Weights",
        &format!("{}M", ternary_weights / 1_000_000),
    ]);
    table.add_row(vec!["Average Sparsity", &format!("{:.2}%", avg_sparsity)]);
    table.add_row(vec!["Average Scale Factor", &format!("{:.6}", avg_scale)]);
    table.add_row(vec![
    ...
    table.add_row(vec![
        Cell::new("OPTIMAL EPOCHS REQUIRED")
            .add_attribute(Attribute::Bold)
            .fg(Color::Green),
        Cell::new(epochs_required.to_string())
            .add_attribute(Attribute::Bold)
            .fg(Color::Green),
    ]);
    let final_prob = trajectory.last().unwrap_or(&model_certainty);
    table.add_row(vec![
    ...
    if args.contains(&"--boost".to_string()) || args.contains(&"--tier3".to_string()) {
        println!("\n  🔥 TIER 3 CALIBRATION ACTIVE: Adaptive PRQ Signal Amplification...");
        println!("  ✅ Tier 3 PRQ Calibration dry-run complete. (Destructive mutations disabled per Phase 10 Roadmap).");
    }
