...
fn ggml_to_safetensors_dtype(ggml_dtype: GgmlDType) -> Dtype {
    match ggml_dtype {
        ...
        _ => panic!("Unsupported ggml dtype for safetensors conversion: {:?}", ggml_dtype),
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    println!("✅ Parsed GGUF metadata. Tensor Count: {}", content.tensor_infos.len());
    ...
    for (name, info) in &content.tensor_infos {
        ...
        let view = TensorView::new(dtype, shape, tensor_data).map_err(|e| anyhow::anyhow!("{:?}", e))?;
    ...
    serialize_to_file(iter, &None, std::path::Path::new(output_path)).map_err(|e| anyhow::anyhow!("{:?}", e))?;
