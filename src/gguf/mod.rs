...
impl GGUFModel {
    ...
    pub fn load(path: &str) -> anyhow::Result<Self> {
        ...
        if &mmap[pos..pos + 4] != b"GGUF" {
            anyhow::bail!("No GGUF magic");
        }
        pos += 4;
        ...
        for _ in 0..kv_count {
            ...
            if key == "general.alignment" {
                if let MetadataValue::Uint32(align) = val {
    ...
    pub fn get_tensor_q4_0(&self, name: &str) -> Option<*const BlockQ4_0> {
        self.tensors
            .get(name)
            .filter(|t| t.t_type == TensorType::Q4_0)
            .map(|t| t.data_ptr as *const BlockQ4_0)
    }
}
...
fn read_metadata_value(
    ...
    mut pos: usize,
    val_type: u32,
) -> anyhow::Result<(MetadataValue, usize)> {
    let val = match val_type {
        ...
        1 => {
            let v = mmap[pos] as i8;
            pos += 1;
            MetadataValue::Int8(v)
        }
        2 => {
            let v = u16::from_le_bytes(mmap[pos..pos + 2].try_into()?);
            pos += 2;
            MetadataValue::Uint16(v)
        }
        3 => {
            let v = i16::from_le_bytes(mmap[pos..pos + 2].try_into()?);
            pos += 2;
        ...
        11 => {
            let v = i64::from_le_bytes(mmap[pos..pos + 8].try_into()?);
            pos += 8;
            MetadataValue::Int64(v)
        }
        12 => {
            let v = f64::from_le_bytes(mmap[pos..pos + 8].try_into()?);
            pos += 8;
