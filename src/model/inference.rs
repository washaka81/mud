use crate::asm::{dequantize_q4_0_row, BlockQ4_0};
use crate::gguf::{GGUFModel, MetadataValue};
use crate::model::tokenizer::Tokenizer;
use crate::model::transformer::{ForgeModel, InferenceWorkspace, TransformerLayer};
...
impl ForgeInference {
    pub fn new(gguf: &GGUFModel, vk_ctx: Arc<VulkanContext>) -> anyhow::Result<Self> {
        ...
        let ffn_hidden = extract_u32(gguf, "MUD2.feed_forward_length")
        ...
        for i in 0..n_layers {
            layers.push(TransformerLayer {
                attn_q_w: gguf
                    .get_tensor_q4_0(&format!("blk.{}.attn_q.weight", i))
                    .unwrap_or(std::ptr::null()),
                attn_q_b: gguf
                    .tensors
                    .get(&format!("blk.{}.attn_q.bias", i))
                    .map(|t| t.data_ptr as *const f32)
                    .unwrap_or(std::ptr::null()),
                attn_k_w: gguf
                    .get_tensor_q4_0(&format!("blk.{}.attn_k.weight", i))
                    .unwrap_or(std::ptr::null()),
                attn_k_b: gguf
                    .tensors
                    .get(&format!("blk.{}.attn_k.bias", i))
                    .map(|t| t.data_ptr as *const f32)
                    .unwrap_or(std::ptr::null()),
                attn_v_w: gguf
                    .get_tensor_q4_0(&format!("blk.{}.attn_v.weight", i))
                    .unwrap_or(std::ptr::null()),
                attn_v_b: gguf
                    .tensors
                    .get(&format!("blk.{}.attn_v.bias", i))
                    .map(|t| t.data_ptr as *const f32)
                    .unwrap_or(std::ptr::null()),
                attn_o_w: gguf
                    .get_tensor_q4_0(&format!("blk.{}.attn_output.weight", i))
                    .unwrap_or(std::ptr::null()),
                attn_o_b: gguf
                    .tensors
                    .get(&format!("blk.{}.attn_output.bias", i))
                    .map(|t| t.data_ptr as *const f32)
                    .unwrap_or(std::ptr::null()),
                ffn_gate_w: gguf
                    .get_tensor_q4_0(&format!("blk.{}.ffn_gate.weight", i))
                    .unwrap_or(std::ptr::null()),
                ffn_up_w: gguf
                    .get_tensor_q4_0(&format!("blk.{}.ffn_up.weight", i))
                    .unwrap_or(std::ptr::null()),
                ffn_down_w: gguf
                    .get_tensor_q4_0(&format!("blk.{}.ffn_down.weight", i))
                    .unwrap_or(std::ptr::null()),
                attn_norm_w: gguf
                    .tensors
                    .get(&format!("blk.{}.attn_norm.weight", i))
                    .map(|t| t.data_ptr as *const f32)
                    .unwrap_or(std::ptr::null()),
                ffn_norm_w: gguf
                    .tensors
                    .get(&format!("blk.{}.ffn_norm.weight", i))
                    .map(|t| t.data_ptr as *const f32)
                    .unwrap_or(std::ptr::null()),
                rms_norm_eps: 1e-8,
            });
        ...
        Ok(Self {
            model,
            tokenizer,
            kv_cache_k: vec![0; kv_cache_size],
            kv_cache_v: vec![0; kv_cache_size],
            kv_scales_k: vec![0.0; scales_size],
            kv_scales_v: vec![0.0; scales_size],
            embd_w: gguf
                .get_tensor_q4_0("token_embd.weight")
                .unwrap_or(std::ptr::null()),
            output_w: gguf
                .get_tensor_q4_0("output.weight")
                .unwrap_or(std::ptr::null()),
            output_norm_w: gguf
                .tensors
    ...
    pub fn generate(&mut self, prompt: &str, max_new_tokens: usize) -> anyhow::Result<()> {
        ...
        for (pos, &token) in prompt_tokens.iter().enumerate() {
            let row_ptr = unsafe {
                self.embd_w
                    .add(token as usize * (self.model.hidden_size / 32))
            };
            unsafe {
            ...
            self.model.decode_step(
                ...
                &mut self.kv_cache_k,
                &mut self.kv_cache_v,
                &mut self.kv_scales_k,
                &mut self.kv_scales_v,
                &mut self.ws,
            );
            tokens.push(token);
        }
        ...
        for pos_offset in 1..=max_new_tokens {
            ...
            unsafe {
                let scale =
                    crate::asm::rms_norm_scale_asm(self.model.hidden_size, x.as_ptr(), 1e-8);
                for (i, x_val) in x.iter().enumerate().take(self.model.hidden_size) {
                    self.ws.row_f32[i] = x_val
                        * scale
                        * (if !self.output_norm_w.is_null() {
                            *self.output_norm_w.add(i)
                        } else {
                            1.0
                        });
                }
                ...
                if !self.output_w.is_null() {
                    for i in 0..self.tokenizer.id_to_token.len() {
                        ...
                        let mut sum = 0.0f32;
                        crate::asm::q4_0_gemv_asm(self.model.hidden_size, self.ws.row_f32.as_ptr(), row_ptr, &mut sum as *mut f32);
                        self.ws.attn_proj[i] = sum; // reusing buffer
                    }
                }
            }

            let sampled_idx = self
                .ws
                .attn_proj
                .iter()
                .enumerate()
