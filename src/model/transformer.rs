...
impl ForgeModel {
    ...
    pub fn decode_step(
        ...
        for (l, layer) in self.layers.iter().enumerate() {
            ...
            self.compute_attention_quantized(
                &ws.q,
                &kv_cache_k[layer_offset..],
                &kv_cache_v[layer_offset..],
                &kv_scales_k[l * scales_layer_offset..],
                &kv_scales_v[l * scales_layer_offset..],
                &mut ws.attn_out,
                pos,
                &mut ws.attn_proj,
            );
            ...
            for (i, item) in x.iter_mut().enumerate().take(self.hidden_size) {
                *item += ws.attn_proj[i];
            }

            ws.ffn_gate.fill(0.0);
            ws.ffn_up.fill(0.0);
            unsafe {
                crate::asm::q4_0_gemv_fused(
                    self.hidden_size,
    ...
    #[allow(clippy::too_many_arguments)]
    fn compute_attention_quantized(
        &self,
        q: &[f32],
        k_cache: &[i8],
        v_cache: &[i8],
        ...
        for h in 0..n_heads {
            ...
            for (p, score_item) in scores.iter_mut().enumerate().take(pos + 1) {
                ...
                for i in 0..head_size {
                    score += q_h[i] * (k_hp_i8[i] as f32 * k_scale);
                }
                *score_item = score * scale;
            }

            let max_score = scores[0..=pos]
                .iter()
                .cloned()
                .fold(f32::NEG_INFINITY, f32::max);
            let mut sum = 0.0f32;
            for p_score in scores.iter_mut().take(pos + 1) {
                *p_score = (*p_score - max_score).exp();
                sum += *p_score;
            }
            ...
            for p in 0..=pos {
                ...
                for i in 0..head_size {
                    out[out_h_start + i] += scores[p] * (v_hp_i8[i] as f32 * v_scale);
                }
            }
        }
    }
    ...
    /// # Safety
    ///
    /// This function is unsafe because it dereferences the `weights` raw pointer.
    pub unsafe fn gemv_pure_rust_no_norm(
        &self,
        n_in: usize,
        n_out: usize,
        input: &[f32],
        ...
        for (i, item) in out.iter_mut().enumerate().take(n_out) {
            ...
            let mut sum = 0.0f32;
            crate::asm::q4_0_gemv_asm(n_in, input.as_ptr(), weight_row_ptr, &mut sum as *mut f32);
            *item = sum;
        }
    }
