...
pub fn dequantize_q4_0_row(row: *const BlockQ4_0, out: &mut [f32], n: usize) {
    let blocks = n / 32;
    unsafe {
        for i in 0..blocks {
            let block = &*row.add(i);
...
pub fn q4_0_gemv_fused(n_in: usize, n_out: usize, x: &[f32], weights: *const BlockQ4_0, norm_w: *const f32, out: &mut [f32], eps: f32) {
    unsafe {
        let scale = rms_norm_scale_asm(n_in, x.as_ptr(), eps);
        let mut x_norm = vec![0.0f32; n_in];
        for i in 0..n_in {
            x_norm[i] = x[i] * scale * (*norm_w.add(i));
        }
