...
pub fn apply_gradient_jitter(grads: &mut SlimeLayerGradients, config: &SelfPlayConfig, step: usize) {
    if config.neural_kick_jitter <= 0.0 { return; }
...
use crate::mud::slime_forward::{SlimeLayer, evaluate_slime_block, apply_output_norm, layer_is_valid};

/// Genera una secuencia autorregresiva (sueño) utilizando el atractor JEPA
#[allow(clippy::too_many_arguments, clippy::not_unsafe_ptr_arg_deref, clippy::needless_range_loop)]
pub fn generate_synthetic_sequence(
    ...
    for gen_pos in 0..config.max_sequence_len {
        ...
        for h in 0..hidden {
            ...
            crate::mud::slime::SlimeRegister::init_from_embed(&mut ws.registers[h], &mut ws.jepa_z, h, ws.hidden_size, ws.num_layers, emb_val, gen_pos == 0);
        }
        
        // 2. Forward pass por todas las capas
        for (l_idx, layer) in layers.iter().enumerate() {
            if !layer_is_valid(layer) { continue; }
...
pub fn is_sequence_confident(
    entropy_per_token: &[f32],
    config: &SelfPlayConfig
) -> bool {
    if entropy_per_token.is_empty() { return false; }
